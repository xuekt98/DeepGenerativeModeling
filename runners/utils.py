...
from dataset_handlers.custom import CustomSingleDataset, CustomAlignedDataset
...
def make_save_dirs(args, prefix, suffix=None, with_time=False):
    ...
    output_path = make_dir(os.path.join(args.output_path, prefix, suffix, time_str))
    image_path = make_dir(os.path.join(output_path, "image"))
    log_path = make_dir(os.path.join(output_path, "log"))
    model_path = make_dir(os.path.join(output_path, "model"))
    sample_path = make_dir(os.path.join(output_path, "samples"))
    sample_to_calc_path = make_dir(os.path.join(output_path, "sample_to_calc"))
    print("create output path " + output_path)
    return image_path, model_path, log_path, sample_path, sample_to_calc_path
...
def get_dataset(dataset_config):
    train_dataset = Registers.datasets[dataset_config.dataset_type](dataset_config, stage='train')
    val_dataset = Registers.datasets[dataset_config.dataset_type](dataset_config, stage='val')
    if dataset_config.has_test_dataset:
        test_dataset = Registers.datasets[dataset_config.dataset_type](dataset_config, stage='test')
    else:
